	 ...
	 ,m_channel(new QWebChannel(this))
{
	...
	QString strEmplyeeID, strPicturePath;
	QString strExternal;
	...
	if (isGroupTalk)
	{
		...
		for (int i = 0; i < rows; i++)
		{
			...
			strEmplyeeID = queryEmployeeModel.data(employeeModelIndex).toString();
			strPicturePath = queryEmployeeModel.data(pictureModelIndex).toString();
			//qDebug() << strPicturePath;

			strExternal = "external_" + strEmplyeeID;
